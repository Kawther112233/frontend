...
const Profile = ({ setReloadPage, reloadPage }) => {
  ...
  const uploadProfileImage = async () => {
    try {
      ...
      formData.append("upload_preset", "kawther");

      const response = await axios.post(
        "https://api.cloudinary.com/v1_1/dfqtw0iq8/upload",
        formData
      );
  ...
  // Cours filtrés en fonction de l'instructeur_id ou du pack_id
  const filteredCourses = courses?.filter(
    (course) =>
      course?.instructor_id === user?._id || course?.pack_id === user?._id
  );
// ---------- pagination --------------
    // Fonction pour gérer les flèches précédentes et suivantes pour les cours
